...
**第一步：全文扫描，列出所有角色名**
- 从头到尾逐行扫描剧本，记录每一个出现过的角色名称（包括对白中被提及的、旁白中出现的、仅在背景中描述的）。
- 角色名可能出现在以下位置：方括号标注如[角色名]或[角色名·年龄·服装]、对白前缀如"角色名："、叙述文本中直接提及。
- **任何有名字的人物都算角色**，哪怕只出现一次、只被提到一次、只在回忆中出现。
- 群众演员/无名路人不需要提取，但有称谓的（如"老板"、"医生张"）需要提取。
- **再次强调：场景名称、地点名称、物品名称不是角色！**
...
2. **服装变体（costumes）提取规则 —— 极其重要，请逐条执行**：
   - **通读全剧本**，查找同一角色是否在不同场景穿着不同的服装/造型/装备。
   - **关键线索1 — 方括号标注**：剧本中角色名常以 [角色名·年龄·服装名] 格式标注。同一角色名如果出现了**不同的后缀**（例如 [NATHAN COLE·32岁·探险装备] 和 [NATHAN COLE·32岁·不合身西装]），则表示该角色有多套服装变体，**全部必须提取**。
   - **关键线索2 — 叙述描写**：即使没有方括号标注，如果剧本叙述中描写了角色换衣、换装、穿上新装备等，也应作为服装变体提取。
   - **关键线索3 — 场景转换**：角色在不同场景/时间段出现时，如果其着装明显不同（如白天穿制服、晚上穿便装），应作为不同服装变体。
   - 如果同一角色有**2套及以上**不同的服装/装扮/造型，则必须提取为 costumes 数组。
   - **年龄×服装交叉**：如果同一角色跨越不同年龄阶段（如少年期→成年期），则每个年龄段的每套服装都必须作为独立变体。label 格式为"年龄段·服装名"，例如"32岁·探险装备"、"32岁·不合身西装"。
...
3. 场景名称应简洁明了。
4. **注意**：场景是地点/环境，不要把人物放到场景里，也不要把场景放到角色里。
